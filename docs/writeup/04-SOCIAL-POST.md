# Social post (one required, must be LINKED from the Space README)

Three variants in Kyle's voice. Pick one, attach the Beat-3 clip or a still showing O'Brien's
precedent read + the 3D risk flag. Keep the honesty line: local, small, retrieval, no overclaim.
No em or en dashes. Space link: https://node.microfactory.space (fallback: https://huggingface.co/spaces/build-small-hackathon/microfactory-lab). Use the custom domain in `[Space link]`.

---

## Variant A: the story lead (recommended)
...
## Variant C: the maker-problem lead
...
## Variant C refinements (for review)

Use these as alternates or combine elements. All keep the maker-problem lead, the local/no-cloud
stance, and the honest "retrieval, not fine-tuning" line.
...
## Build Small / channel post (meta framing)

Use this for a longer post on LinkedIn, your newsletter, or a build thread. It explains the judging lens and the discipline behind the demo.

> A clever proof of concept that works — not a production system.
>
> Backyard AI judges a demo video, a writeup, a plausible "someone could use it" story, and a working Gradio app. Build the judged things well; keep everything else minimal. **A component nobody judges should never cost more than a component that is judged.** Simplify as you go.
>
...
**Ship list:** C1 and C2 are the chosen variants. Combine the tighter opener of C1 with the agent
mechanics of C2, or post one on X/Threads and the other on LinkedIn. The channel post above can run as a standalone build-thread opener or after the demo clip.
